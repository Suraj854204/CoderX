...
of backend — only the internals swap. Backend is selected via CODERX_BACKEND:
...
# Override with: set CODERX_MODEL=<ollama tag>
...
MODEL_NAME = os.environ.get("CODERX_MODEL", "qwen3:4b-instruct")



# "ollama" (default, always works) or "qnn" (production path, venue-hardware only)

BACKEND = os.environ.get("CODERX_BACKEND", "qnn").lower()



# Set via env var CODERX_MOCK_LLM=1 to bypass the backend entirely and return
...
# Works identically regardless of CODERX_BACKEND, so mock mode is always

# available as a safety net per Section 19.7.

MOCK_MODE = os.environ.get("CODERX_MOCK_LLM", "0") == "1"
...
GENIE_MODEL = os.environ.get("CODERX_GENIE_MODEL", "ai-hub-models/Qwen3-4B-Instruct-2507")

GENIE_COMPUTE = os.environ.get("CODERX_GENIE_COMPUTE", "npu")

GENIE_THINK = os.environ.get("CODERX_GENIE_THINK", "false")
...
# Override with CODERX_GENIE_PROMPT_FILE if needed.

GENIE_PROMPT_FILE = os.environ.get(

    "CODERX_GENIE_PROMPT_FILE",

    os.path.join(os.path.dirname(os.path.abspath(__file__)), "coderx_geniex_prompt.txt"),
...
# can take well over a minute. Override with: set CODERX_TIMEOUT=600

DEFAULT_TIMEOUT = int(os.environ.get("CODERX_TIMEOUT", "420"))
...
def review_hunk(prompt: str, model: str = MODEL_NAME, timeout: int = DEFAULT_TIMEOUT) -> LLMResult:
    ...
    needs to change when CODERX_BACKEND switches from "ollama" to "qnn".



    If MOCK_MODE is on (CODERX_MOCK_LLM=1), skips the backend entirely and
...
def _review_hunk_ollama(prompt: str, model: str, timeout: int) -> LLMResult:
    ...
    network stall) is silent until CODERX_TIMEOUT finally fires — which
