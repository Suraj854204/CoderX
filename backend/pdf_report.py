...
fpdf2-based PDF renderer for the CoderX review report — the real
...
def _status_strip(pdf: CoderXReportPDF, commit_info, counts: dict, model_name: str, device_name: str):
    x, y = MARGIN, MARGIN
    ...
    pdf.cell(0, 7, "CoderX - On-Device Review Report")
...
def _section_heading(pdf: CoderXReportPDF, y: float, severity: str, count: int) -> float:
    x = MARGIN
...
def _measure_finding_height(pdf: CoderXReportPDF, d) -> float:
...
def _render_finding(pdf: CoderXReportPDF, y: float, d) -> float:
    f = d.reviewed_finding.finding
    status = _decision_status(d)
...
def _empty_state(pdf: CoderXReportPDF, y: float):
    pdf.set_draw_color(*PANEL_BORDER)
...
def _footer_privacy_line(pdf: CoderXReportPDF):
    y = pdf.h - 26
    pdf.set_draw_color(*PANEL_BORDER)
    ...
    pdf.set_text_color(*TEXT_FAINT)
    pdf.cell(CONTENT_W / 2, 5, "CoderX - Qualcomm Multiverse Hackathon")
...
def render_pdf(
    decisions: List,
    commit_info=None,
    output_path: str = "coderx_report.pdf",
    model_name: str = "Qwen-4B-Instruct-2507"
    "",
    device_name: str = "OnePlus 15",
) -> str:
    pdf = CoderXReportPDF(format="A4")
