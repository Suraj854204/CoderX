...
# Bumped whenever PROMPT_TEMPLATE's wording changes in a way that could
# change model output for the same diff. review_cache.py folds this into
# the cache key, so an edited prompt automatically invalidates old cached
# verdicts instead of silently serving findings produced by a different
# instruction set.
