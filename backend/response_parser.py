...
class Finding:
    ...
    # --- Provenance metadata (populated by run_review.py AFTER parsing,
    # never by the parser itself, and never invented) ---
    # category: derived deterministically from risk_router's matched signal
    # names for this hunk (e.g. "auth, sql"), or "general" for a STANDARD
    # hunk, or "trivial" for a fast-path-skipped one. This is OUR
    # classification of the hunk, not a claim the model categorized itself.
    category: str = ""
    # inference_path: exactly one of "npu:qnn", "cpu:ollama", "mock",
    # "cache" (verdict reused, no call made this run), or "fast-path-skip"
    # (no LLM call at all — synthetic finding). Always reflects what
    # actually happened for this specific finding.
    inference_path: str = ""
    # model: the literal model identifier used, e.g.
    # "ai-hub-models/Qwen3-4B-Instruct-2507" or "qwen3:4b-instruct".
    # Empty for fast-path-skip findings, since no model was invoked.
    model: str = ""
    # confidence: Qwen3-4B-Instruct-2507 is NOT prompted to emit a
    # confidence score, and CoderX does not compute one post-hoc (e.g. via
    # log-prob margins) in this version — GenieX's one-shot CLI interface
    # does not expose token log-probs. So this is intentionally None,
    # never a fabricated number. confidence_note explains why, and is
    # shown in the UI/report instead of a bare "N/A" so it reads as a
    # deliberate design choice, not a missing feature.
    confidence: Optional[float] = None
    confidence_note: str = (
        "Not calibrated — the model is not prompted for a confidence "
        "score and none is computed post-hoc in this version."
    )
...
def parse_findings(raw_output: str) -> List[Finding]:
    ...
    _log = get_logger("coderx.response_parser")
