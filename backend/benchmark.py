...
Benchmark harness for the CoderX review pipeline — Section 9 / Section 20
...
Respects the same env vars as llm_client.py (CODERX_MODEL, CODERX_MOCK_LLM,
CODERX_TIMEOUT, CODERX_BACKEND) so this can be run once against Ollama/Phi-3
...
def _backend_label() -> str:
    if MOCK_MODE:
        return "mock"
    backend = os.environ.get("CODERX_BACKEND", "ollama")
...
def main():
    parser = argparse.ArgumentParser(description="CoderX review pipeline benchmark harness")
