...
Centralized logging for the CoderX backend + hooks-invoked components
...
- Every log line is automatically tagged with which commit it's about,
  ...
  every function call) — so `grep c5eb95a logs/coderx.log` shows the
...
CONFIGURATION (env vars):
    CODERX_LOG_LEVEL=DEBUG|INFO|WARNING|ERROR   (default: INFO)
    CODERX_LOG_DIR=<path>                        (default: ./logs, relative to cwd)

Every log record goes to BOTH the console (same stdout everything already
printed to before) and a persistent file at <CODERX_LOG_DIR>/coderx.log
...
def _configure_once() -> None:
    ...
    with _configure_lock:
        ...
        level_name = os.environ.get("CODERX_LOG_LEVEL", "INFO").upper()
        level = getattr(logging, level_name, logging.INFO)

        log_dir = os.environ.get("CODERX_LOG_DIR", "logs")
        os.makedirs(log_dir, exist_ok=True)
        log_path = os.path.join(log_dir, "coderx.log")
...
def get_logger(name: str) -> logging.Logger:
    ...
    to write to both console and the shared logs/coderx.log file, with
    the current commit automatically tagged on every line.
    """
    _configure_once()
    # Component loggers nest under "coderx." so they share the handlers
    # configured above (via propagation to the "coderx" logger) while
    # still reporting their own module name in each line.
    return logging.getLogger(f"coderx.{name}")
