...
def _current_model_device_labels() -> tuple:
    ...
    if llm_client.MOCK_MODE:
        return "Mock LLM (CODERX_MOCK_LLM=1)", "N/A (mock mode)"
...
def _write_text_report(
    ...
    lines = []
    lines.append(f"CoderX Review Report — {datetime.now().strftime('%B %d, %Y %H:%M')}")
    if commit_info:
...
def _write_json_audit_trail(
    ...
    """
    Machine-readable sibling of the PDF/text report — the full
    human-in-the-loop audit trail as structured data: every finding, its
    provenance (category/inference_path/model, from response_parser's
    Finding metadata), the developer's decision, and — for false
    positives — their reasoning plus the model's reiteration verdict.

    This is the artifact a CI system, a second tool, or a judge script
    could actually parse, rather than needing to re-extract facts from
    PDF/text prose. Nothing here is computed or inferred — every field is
    copied directly from the FindingDecision/Finding objects already
    ...
    for d in decisions:
        ...
        if d.decision == "false_positive":
            record["developer_comment"] = d.dev_comment
            record["llm_verdict"] = d.llm_verdict or None
            record["llm_reiteration"] = d.llm_reiteration or None
        records.append(record)

    payload = {
        "commit": {
            "id": commit_info.commit_id if commit_info else None,
            "short_id": commit_info.short_id if commit_info else None,
            "author": commit_info.author_name if commit_info else None,
...
def generate_report(
    ...
    output_path: str = "coderx_report.txt",
) -> str:
    """
    Writes the PDF (primary — this is the path returned), a plain-text
    sibling, and a JSON audit-trail sibling for the given decisions.
    ...
    existing call sites (report_trigger.py passes "coderx_report_{short_id}.txt")
    keep working unmodified — a .pdf, .txt, AND .json get written next to it.
    """
    ...
    with stage(log, "write_json_audit_trail"):
        json_path = _write_json_audit_trail(decisions, commit_info, f"{base}.json", model_name, device_name)
    ...
    log.info(f"Wrote {json_path} (structured audit trail)")
