...
# writing timestamped checkpoints to logs/coderx_hooks.log alongside the
...
LOG_FILE="$SCRIPT_DIR/logs/coderx_hooks.log"
...
if curl -s -o /dev/null -m 2 http://localhost:8000/health; then
    ...
    nohup python -m uvicorn webhook_server:app --host 0.0.0.0 --port 8000 > "$SCRIPT_DIR/logs/coderx_webhook.log" 2>&1 &
    WEBHOOK_PID=$!
    sleep 1
    if curl -s -o /dev/null -m 2 http://localhost:8000/health; then
        log_line "      Started and confirmed healthy (PID $WEBHOOK_PID). Logs: $SCRIPT_DIR/logs/coderx_webhook.log"
    else
        log_line "      WARNING: started (PID $WEBHOOK_PID) but health check did not respond within 1s — check $SCRIPT_DIR/logs/coderx_webhook.log for a startup error."
    fi
fi
...
if [ ! -d "mobile" ]; then
    ...
    nohup npx expo start --port 5678 > "$SCRIPT_DIR/logs/coderx_expo.log" 2>&1 &
    EXPO_PID=$!
    sleep 3
    log_line "      Started on port 5678 (PID $EXPO_PID). QR code/dev server URL: $SCRIPT_DIR/logs/coderx_expo.log"
    echo "      View it now with: cat \"$SCRIPT_DIR/logs/coderx_expo.log\""
...
echo "  - Scan the QR code to open the mobile app: cat \"$SCRIPT_DIR/logs/coderx_expo.log\""
echo "  - Findings stream over ws://0.0.0.0:8765 to the mobile app (if running)."
echo "  - Full structured logs (Python side): $SCRIPT_DIR/backend/logs/coderx.log"
