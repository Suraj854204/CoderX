...
function flattenPayload(payload) {
  ...
  return payload.findings.map((f) => ({
    ...
    fix: f.fix,
    category: f.category || "",
    inferencePath: f.inference_path || "",
    model: f.model || "",
...
// Maps an inference_path value from the backend to what the badge shows.
// Deliberately literal — never implies a cloud fallback exists, since
// none does. "fast-path-skip" and "cache" both mean "no model call
// happened this run", which is exactly as accurate a label as "on-device".
function inferencePathLabel(inferencePath) {
  switch (inferencePath) {
    case "npu:qnn":
      return "● LOCAL AI \u2014 Snapdragon NPU";
    case "cache":
      return "● LOCAL AI \u2014 cached verdict (no NPU call)";
    case "fast-path-skip":
      return "\u25CB No model call \u2014 fast-path skip";
    default:
      if (inferencePath && inferencePath.startsWith("cpu:")) {
        return `● LOCAL AI \u2014 CPU (${inferencePath.slice(4)}, dev/fallback)`;
      }
      return "";
  }
}
...
export default function App() {
  ...
  const sendMessage = useCallback((message) => {
    ...
    if (ws && ws.readyState === WebSocket.OPEN) {
      ...
      console.log("[CoderX] Socket not open — message not sent:", message);
    }
  }, []);
    ...
    const connect = () => {
      ...
      ws.onmessage = (event) => {
        ...
        try {
          ...
          console.warn("[CoderX] Parse error:", e);
          return;
        }
        if (!mounted) return;
      ...
      ws.onerror = (e) => {
        console.log("[CoderX] ERROR:", e.message || e);
      };
  ...
  const renderItem = ({ item }) => {
    ...
    return (
      <View style={[styles.card, { backgroundColor: sev.bg, borderLeftColor: sev.color }]}>
        ...
        {!!inferencePathLabel(item.inferencePath) && (
          <Text style={styles.inferenceBadge}>
            {inferencePathLabel(item.inferencePath)}
  ...
  return (
    <SafeAreaView style={styles.safeArea}>
      ...
      <View style={styles.header}>
        <Text style={styles.headerTitle}>CoderX</Text>
